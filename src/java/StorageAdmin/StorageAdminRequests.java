 ...
 * @author RAHIL
 */
@WebServlet(name = "StorageAdminRequests", urlPatterns = {"/StorageAdmin/Requests"})
public class StorageAdminRequests extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int warId=(Integer)htSess.getAttribute("warId");
        String managerEmail=(String)htSess.getAttribute("user_email");
        try{
            sess=HibernateUtil.getSessionFactory().openSession();
            tx=sess.beginTransaction();
            List<Supplies> supplies_list =(List<Supplies>) sess.createCriteria(Supplies.class).add(Restrictions.ge("supplyDate", Date.valueOf(LocalDate.now() ) ) ).add(Restrictions.eq("desId",0)).list();
            List<Fruits> fruit_list=(List<Fruits>) sess.createCriteria(Fruits.class).list();
            
            
            
            List<Inventory> inv_list =(List<Inventory>) sess.createCriteria(Inventory.class).add(Restrictions.eq("wareHouseId", warId)).list();
...
//functional operation            
        ...
        if(redirection_string!=null)
            ...
            RequestDispatcher red=request.getRequestDispatcher("/templates/StorageAdmin/StorageAdminRequests.jsp");
